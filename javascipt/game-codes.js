(function () {
    ...
    function applyLastUpdatedFromMeta(database) {
        ...
        const gameUpdated = cleanText(meta && meta.gameLastUpdated && gameId ? meta.gameLastUpdated[gameId] : "");
        const globalTested = cleanText(meta && meta.lastTested);
        const gameTested = cleanText(meta && meta.gameLastTested && gameId ? meta.gameLastTested[gameId] : "");
    ...
    function injectStructuredData(database) {
        ...
        const meta = database && typeof database._meta === "object" ? database._meta : null;
        const globalUpdated = cleanText(meta && meta.lastUpdated);
        const gameUpdated = cleanText(meta && meta.gameLastUpdated && meta.gameLastUpdated[gameId]);
    ...
    function sanitizeGameData(data) {
        ...
        return {
            workingCodes: Array.isArray(source.workingCodes) ? source.workingCodes : [],
            expiredCodes: Array.isArray(source.expiredCodes) ? source.expiredCodes : []
        };
    }
    ...
    function renderWorkingCodes(listElement, workingCodes) {
        ...
        if (workingCodes.length === 0) {
            listElement.innerHTML = '<li class="list-group-item">No active codes right now.</li>';
            return;
        }
    ...
    function renderSimpleList(listElement, values, itemClassName) {
        listElement.innerHTML = "";
    ...
    function renderGamePage(gameData) {
        ...
        if (expiredCodesList) {
            renderSimpleList(expiredCodesList, gameData.expiredCodes, "");
        }
    }
